...
# -----------------------------------
# The vendored source is NOT in this context.
#
# deps/ and .git/modules/ are excluded by the image's ignorefile (generated by
# makefiles/30-build.mk), because the image has no use for either: the binaries
# are built in the deps container, and 1.1G of source plus 2.8G of submodule
# history bought nothing but provenance.
#
# Provenance now ships as /usr/immutablue/deps/dep_info.json -- every
# dependency's remote and exact commit -- which is enough to clone and read the
# source on demand, and is what immutablue-crash does into a crash-analysis
# directory. That replaced ~3.9G of context with a few kilobytes.
#
# This is also why there is no longer any git-shallowing here. That routine
# existed to make the shipped histories affordable; nothing ships them now.
# -----------------------------------
