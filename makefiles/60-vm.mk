...
lima:
	...
	@echo "" >> $(LIMA_YAML)
	@echo "# No provision scripts — kuberblue handles its own boot sequence" >> $(LIMA_YAML)
	@echo "provision: []" >> $(LIMA_YAML)
	@echo "" >> $(LIMA_YAML)
	@echo "# Lima guest agent installs to ~/.local/bin (mutable in ostree)." >> $(LIMA_YAML)
	@echo "# This lets Lima's hostagent receive the 'running' event so limactl start" >> $(LIMA_YAML)
	@echo "# completes normally. plain: true would skip the guest agent and cause a" >> $(LIMA_YAML)
	@echo "# 600s timeout since Lima never receives the event." >> $(LIMA_YAML)
	@echo "" >> $(LIMA_YAML)
