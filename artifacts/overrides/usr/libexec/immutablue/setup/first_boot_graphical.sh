...
then
    ...
    then 
        ...
        ptyxis -- bash -c 'echo "Please connect to the internet and then restart to finish installation." && read -p "hit inter to continure, reboot when ready (after setting up internet)"'
        exit 1
    fi
fi
...
then
    ...
    ptyxis -- bash -c 'immutablue install && echo "please reboot one last time" && read -p "hit enter to continue. reboot when ready"'
fi
