...
# Wrapper around `podman run --rm` with retry logic to handle transient
# attach-socket failures in nested container CI environments.
podman_run () {
    local max_retries=3
    local attempt=0

    while (( attempt < max_retries )); do
        if podman run --rm "$@"; then
            return 0
        fi
        ((attempt++))
        if (( attempt < max_retries )); then
            echo "podman run attempt $attempt failed, retrying in 2s..." >&2
            sleep 2
        fi
    done
    return 1
...
function test_just_commands() {
    ...
    if ! podman_run "$IMAGE" command -v just >/dev/null 2>&1; then
    ...
    if ! podman_run "$IMAGE" test -f "$kuberblue_justfile"; then
    ...
    if ! podman_run "$IMAGE" just --list-heading="Available Kuberblue commands:" --justfile="$kuberblue_justfile" --list; then
    ...
    for cmd in "${expected_commands[@]}"; do
        ...
        if ! podman_run "$IMAGE" just --justfile="$kuberblue_justfile" --summary | grep -q "$cmd"; then
    ...
    if ! podman_run "$IMAGE" just --justfile="$kuberblue_justfile" --summary >/dev/null 2>&1; then
...
function test_manifest_deployment() {
    ...
    if ! podman_run "$IMAGE" test -d "$manifests_dir"; then
    ...
    for manifest_file in "${expected_files[@]}"; do
        if podman_run "$IMAGE" test -f "$manifest_file"; then
    ...
    if podman_run "$IMAGE" test -d "$manifests_dir/00-infrastructure/10-openebs"; then
        echo "Found OpenEBS manifest directory"
        # Check for expected OpenEBS files
        podman_run "$IMAGE" test -f "$manifests_dir/00-infrastructure/10-openebs/00-metadata.yaml" && echo "Found OpenEBS metadata file"
        podman_run "$IMAGE" test -f "$manifests_dir/00-infrastructure/10-openebs/10-values.yaml" && echo "Found OpenEBS values file"
    fi
    ...
    for yaml_file in "${yaml_files[@]}"; do
        if podman_run "$IMAGE" test -f "$yaml_file"; then
            echo "Testing YAML syntax for: $yaml_file"
            # Basic YAML syntax test using python (available in container)
            if ! podman_run "$IMAGE" python3 -c "import yaml; yaml.safe_load(open('$yaml_file'))" 2>/dev/null; then
    ...
    if podman_run "$IMAGE" test -f "$manifests_dir/metadata.yaml.tpl"; then
        echo "Found metadata template file"
        if ! podman_run "$IMAGE" test -s "$manifests_dir/metadata.yaml.tpl"; then
    ...
    if podman_run "$IMAGE" test -f "$deploy_script"; then
        echo "Found deployment script: $deploy_script"
        # Test script syntax
        if ! podman_run "$IMAGE" bash -n "$deploy_script"; then
...
function test_user_management() {
    ...
    if ! podman_run "$IMAGE" test -f "$user_script"; then
        echo "FAIL: User creation script not found at $user_script"
        return 1
    fi
    
    # Validate script syntax
    if ! podman_run "$IMAGE" bash -n "$user_script"; then
    ...
    if ! podman_run "$IMAGE" test -f "$variables_file"; then
        echo "FAIL: Variables file not found at $variables_file"
        return 1
    fi
    
    # Test variables file syntax
    if ! podman_run "$IMAGE" bash -n "$variables_file"; then
    ...
    if ! podman_run "$IMAGE" bash -c "source $variables_file && test -n \"\$(kuberblue_uid)\""; then
    ...
    local uid_value
    uid_value=$(podman_run "$IMAGE" bash -c "source $variables_file && kuberblue_uid")
    ...
    if ! podman_run "$IMAGE" test -f "$config_script"; then
        echo "FAIL: Kubectl config script not found at $config_script"
        return 1
    fi
    
    # Validate script syntax
    if ! podman_run "$IMAGE" bash -n "$config_script"; then
    ...
    if ! podman_run "$IMAGE" test -f "$get_config_script"; then
        echo "FAIL: Get config script not found at $get_config_script"
        return 1
    fi
    
    # Validate script syntax
    if ! podman_run "$IMAGE" bash -n "$get_config_script"; then
    ...
    if ! podman_run "$IMAGE" test -f "$run_as_user_script"; then
        echo "FAIL: Run as user script not found at $run_as_user_script"
        return 1
    fi
    
    # Validate script syntax
    if ! podman_run "$IMAGE" bash -n "$run_as_user_script"; then
    ...
    if ! podman_run "$IMAGE" test -f "/etc/sudoers"; then
...
function test_script_functionality() {
    ...
    if ! podman_run "$IMAGE" test -d "$scripts_dir"; then
    ...
    if ! podman_run "$IMAGE" test -f "$common_script"; then
    ...
    for script_file in "${key_scripts[@]}"; do
        if podman_run "$IMAGE" test -f "$script_file"; then
            echo "Checking syntax of $script_file"
            if ! podman_run "$IMAGE" bash -n "$script_file"; then
...
function test_configuration_files() {
    ...
    if ! podman_run "$IMAGE" test -d "$config_dir"; then
    ...
    if ! podman_run "$IMAGE" test -f "$cluster_config"; then
        echo "FAIL: Cluster configuration not found at $cluster_config"
        return 1
    fi

    # Validate YAML syntax
    if ! podman_run "$IMAGE" python3 -c "import yaml; list(yaml.safe_load_all(open('$cluster_config')))" 2>/dev/null; then
    ...
    for config in "${system_configs[@]}"; do
        if podman_run "$IMAGE" test -f "$config"; then
